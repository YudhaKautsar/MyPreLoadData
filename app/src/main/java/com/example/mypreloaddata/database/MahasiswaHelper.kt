...
class MahasiswaHelper(context: Context) {
    ...
    fun insert(mahasiswaModel: MahasiswaModel): Long {
        ...
        initialValue.put(NAMA, mahasiswaModel.name)
        initialValue.put(NIM, mahasiswaModel.nim)
        return database.insert(TABLE_NAME, null, initialValue)
    }

    fun getDataByName(name: String): ArrayList<MahasiswaModel> {
        val cursor = database.query(TABLE_NAME, null, "$NAMA LIKE ?", arrayOf(name), null, null, "$ID ASC", null)
        cursor.moveToFirst()
        val arrayList = ArrayList<MahasiswaModel>()
        var mahasiswaModel: MahasiswaModel
        if (cursor.count > 0) {
